...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> (int, int):
    ...
    """
    gold = df([x.split(',') for line in gold_prices.splitlines(keepends=False) for x in line.split()])
    gold = gold.rename(columns={0: 'Date', 1: 'GoldPrice'})

    gold['Year'] = gold['Date'].apply(lambda x: int(x[:4])).astype(int)
    gold['PriceDiff'] = gold['GoldPrice'].astype(float).diff()
    gold = gold.sort_values(by='PriceDiff').reset_index()
    return gold.loc[0, 'Year'], gold.loc[len(gold) - 2, 'Year']
